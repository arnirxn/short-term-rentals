...
def write_visualisations_to_local_dir(df=None, directory=None):
    ...
    # Get and clean data
    df = df if df else pd.read_csv(os.path.join(DATA_SOURCE_BASE_URL, "listings.csv.gz"), compression="gzip")
    clean_dataframe(df)
    ...
    create_pie_chart(df, "host_type", title='Percentage of listings by host type', legend_title="Is Superhost", viz_dir=directory)
